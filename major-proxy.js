...
class GLaDOS {
  ...
  async checkProxyIP(proxy) {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new Error(
          `Tidak dapat memeriksa IP proxy. Status code: ${response.status}`,
        );
      }
    } catch (error) {
  ...
  async processAccount(accountData, proxy) {
    ...
    try {
      ...
      await this.log(
        `Tidak dapat memeriksa IP proxy: ${error.message}`,
        "warning",
      );
    }
...
if (isMainThread) {
  ...
  const glados = new GLaDOS();
  glados
    .processAccount(workerData.account, workerData.proxy)
    .then(() => {
